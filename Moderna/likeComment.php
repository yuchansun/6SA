...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($input['commentId']) || empty($input['commentId'])) {
        echo json_encode(['success' => false, 'message' => '缺少 commentId']);
        exit;
    }
    ...
    if ($checkCommentLikeResult->num_rows > 0) {
        // Unlike the comment
        $deleteLikeQuery = $conn->prepare("DELETE FROM likes WHERE User_ID = ? AND Comment_ID = ?");
        $deleteLikeQuery->bind_param("ii", $userId, $commentId);
        ...
        echo json_encode(['success' => true, 'newLikesCount' => $comment['Likes'], 'liked' => false]);
        exit;
    }
    ...
    if ($query->execute()) {
        ...
        echo json_encode(['success' => true, 'newLikesCount' => $comment['Likes'], 'liked' => true]);
    } else {
